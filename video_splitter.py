...
def extract_frames_from_video_ffmpeg(in_file_path, frames_out_path):
	print("video_splitter::extract_frames_from_video_ffmpeg, input:, output:", in_file_path, frames_out_path)
	...
	mid_path = frames_out_path+'/'+folder_name
...
#	print("f out path:", f_out_path)

	#ffmpeg command to extract frames
	out_path = f_out_path+'/'+"frame-"
...
def extract_frames_from_video_CV(in_file_path, frames_out_path):
	print("video_splitter::extract_frames_from_video_CV, input path:, output path:", in_file_path, frames_out_path)

	cap = cv2.VideoCapture(in_file_path)

	#parse input file path to create output path
#	print("in frame ext",in_file_path)
	dir_name, file_name = os.path.split(in_file_path)
	sub_folder_name = ".".join(file_name.split('.')[:-1])
	folder_name, num = sub_folder_name.split('-')
...
#		print("final_output_path:",comp_out_path)
...
def split_video_random(file_path, start_pos, split_length, out_path):
	print("video_splitter::split_video_random, file_path:, start_pos:, split_length:, out_path:", file_path, start_pos, split_length, out_path)
...
def video_clip_split(args):
	print("video_splitter::video_clip_split")
